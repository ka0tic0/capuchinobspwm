...
install_yay_and_aur() {
    log_header "3. Configuración de AUR Helper (yay)"
    ...
    if command -v yay &>/dev/null; then
        ...
        if ask_yes_no "¿Deseas compilar e instalar el helper de AUR 'yay'?" "Y"; then
            log_info "Preparando compilación de yay como usuario '$TARGET_USER'..."

            # Garantizar que el usuario tenga permisos en sudoers temporalmente si es necesario
            local tmp_yay_dir="/tmp/yay_build_$$"
            rm -rf "$tmp_yay_dir"
            mkdir -p "$tmp_yay_dir"
            chown -R "$TARGET_USER:$TARGET_GROUP" "$tmp_yay_dir"

            # Clonar y compilar como usuario sin privilegios
            sudo -u "$TARGET_USER" git clone https://aur.archlinux.org/yay.git "$tmp_yay_dir"
            (
                cd "$tmp_yay_dir"
            ...
            if command -v yay &>/dev/null; then
                ...
                log_error "Falló la instalación de yay. Continuando sin AUR."
            fi
        fi
    fi
    ...
    if [[ "$install_yay" == true ]]; then
        ...
        if ask_yes_no "• ¿Instalar temas GTK e Iconos Catppuccin (catppuccin-gtk-theme-mocha y tela-circle-icon-theme)?" "Y"; then
            aur_packages+=(catppuccin-gtk-theme-mocha tela-circle-icon-theme-git)
        fi
        ...
        if [[ ${#aur_packages[@]} -gt 0 ]]; then
            ...
            sudo -u "$TARGET_USER" yay -S --needed --noconfirm "${aur_packages[@]}" || log_warn "Algunos paquetes AUR fallaron, continuando..."
        fi
    fi
}
