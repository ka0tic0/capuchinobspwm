...
# ------------------------------------------------------------------------------
# 6. CAPTURAS DE PANTALLA (SCROT)
# ------------------------------------------------------------------------------
# Pantalla completa
Print
    scrot ~/Pictures/Screenshot_%Y-%m-%d_%H-%M-%S.png -e 'notify-send "Captura" "Guardada en $f"'
# Seleccionar área
super + Print
    scrot -s ~/Pictures/Screenshot_%Y-%m-%d_%H-%M-%S.png -e 'notify-send "Captura de Área" "Guardada en $f"'
EOF
...
configure_polybar() {
    log_info "Configurando Polybar con paleta Catppuccin Mocha y estilos adaptables..."
    
    # 1. Configuración Principal de Polybar
    local POLYBAR_CONF="$CONFIG_DIR/polybar/config.ini"

    cat << 'EOF' > "$POLYBAR_CONF"
; ==============================================================================
; POLYBAR CONFIGURATION - CATPPUCCIN MOCHA THEME
; Adaptado de https://github.com/the-anonymous-raven/polybar-themes
; ==============================================================================
...
modules-center = date
modules-right = cpu memory filesystem pulseaudio network battery
...
EOF
    ...
    # 2. Estilo 2: Polybar Floating Rounded Blocks (Inspirado en the-anonymous-raven/polybar-themes)
...
EOF
    ...
    log_success "Polybar configurada con paleta Catppuccin Mocha y temas modulares."
}
...
setup_custom_scripts() {
    ...
    # Script: launch_polybar (Detecta monitores y lanza la barra en todos los monitores activos)
...
if [ -d "$WALL_DIR" ]; then
    # Buscar recursivamente imágenes png, jpg, jpeg o webp
...
EOF
    ...
    # Script: polybar_theme_selector (Permite cambiar de temas de Polybar dinámicamente)
    local THEME_SELECT="$BIN_DIR/polybar_theme_selector"
    cat << 'EOF' > "$THEME_SELECT"
#!/usr/bin/env bash
CHOSEN=$(echo -e "1. Catppuccin Mocha (Full Default)\n2. Floating Rounded Bar\n3. Recargar Barra Actual" | rofi -dmenu -i -p "Seleccionar Estilo Polybar:" -theme ~/.config/rofi/config.rasi)

case "$CHOSEN" in
    *1.*)
        killall -q polybar
        polybar -c ~/.config/polybar/config.ini main &
        notify-send "Polybar" "Tema Catppuccin Default aplicado"
        ;;
    *2.*)
        killall -q polybar
        polybar -c ~/.config/polybar/styles/floating.ini main &
        notify-send "Polybar" "Tema Floating Rounded aplicado"
        ;;
    *3.*)
        ~/.local/bin/launch_polybar
        notify-send "Polybar" "Polybar recargada"
        ;;
esac
EOF
...
EOF
    ...
    chmod +x "$LAUNCH_POLY" "$WALL_SCRIPT" "$THEME_SELECT" "$NOTIFY_VOL" "$STATUSBAR_LAUNCH"
    chown -R "$TARGET_USER:$TARGET_USER" "$BIN_DIR"
    log_success "Scripts personalizados creados con permisos de ejecución."
}
