...
fix_permissions() {
    if [[ -n "$TARGET_USER" && -d "$TARGET_HOME" ]]; then
        chown -R "$TARGET_USER:$TARGET_GROUP" \
            ...
            "$TARGET_HOME/.bashrc" \
            "$TARGET_HOME/.xinitrc" \
            "$TARGET_HOME/.Xresources" 2>/dev/null || true
    fi
}
...
setup_rices_engine() {
    ...
    # --------------------------------------------------------------------------
    # 5.1 Definición de cada Rice (Paletas Completas)
    # --------------------------------------------------------------------------
...
modules-left = theme_swap xworkspaces
modules-center = xwindow
modules-right = pulseaudio memory cpu network battery date powermenu
...
EOF
    ...
    cat << 'EOF' > "$rices_dir/Nord/polybar.ini"
[bar/main]
monitor = ${env:MONITOR:}
width = 100%
height = 28pt
...
modules-center = date
modules-right = pulseaudio memory cpu network battery powermenu
cursor-click = pointer
enable-ipc = true
wm-restack = bspwm
override-redirect = false
...
PEACH="#ffb86c"
TEAL="#8be9fd"
WALLPAPER_URL="https://raw.githubusercontent.com/dracula/wallpaper/master/dracula-mountains.png"
EOF
...
module-margin = 1
separator = "::"
separator-foreground = ${colors:alt_accent}
font-0 = "JetBrains Mono Nerd Font:size=11;3"
font-1 = "JetBrains Mono Nerd Font:style=Bold:size=12;3"
font-2 = "Font Awesome 6 Free:style=Solid:size=12;3"
modules-left = theme_swap xworkspaces xwindow
...
EOF
    ...
    cat << 'EOF' > "$rices_dir/Doombox/polybar.ini"
[bar/main]
monitor = ${env:MONITOR:}
width = 96%
height = 36pt
...
font-1 = "JetBrains Mono Nerd Font:style=Bold:size=12;3"
font-2 = "Font Awesome 6 Free:style=Solid:size=12;3"
modules-left = powermenu theme_swap xworkspaces
modules-center = xwindow
modules-right = pulseaudio cpu memory network battery date
cursor-click = pointer
enable-ipc = true
wm-restack = bspwm
override-redirect = false
...
EOF
    ...
    cat << 'EOF' > "$rices_dir/Forest/polybar.ini"
[bar/main]
monitor = ${env:MONITOR:}
width = 60%
height = 32pt
...
EOF
    ...
    cat << 'EOF' > "$rices_dir/Horizon/polybar.ini"
[bar/main]
monitor = ${env:MONITOR:}
bottom = true
width = 94%
...
line-size = 2pt
border-size = 1pt
border-color = ${colors:alt_accent}
padding-left = 2
padding-right = 2
module-margin = 1
separator = "-"
...
font-1 = "JetBrains Mono Nerd Font:style=Bold:size=12;3"
font-2 = "Font Awesome 6 Free:style=Solid:size=12;3"
modules-left = powermenu xworkspaces theme_swap
modules-center = xwindow
...
EOF
    ...
    # Descargar galería de wallpapers
    log_info "Descargando galería de wallpapers de zoddDev y fondos temáticos..."
    ...
    # Descargar wallpapers individuales de cada rice
    ...
    curl -sL "https://raw.githubusercontent.com/dracula/wallpaper/master/dracula-lake.png" -o "$wallpapers_dir/Dracula.png" || true
    curl -sL "https://raw.githubusercontent.com/AngelJumbo/gruvbox-wallpapers/main/wallpapers/anime-girl-gruvbox.png" -o "$wallpapers_dir/Gruvbox.png" || true
    curl -sL "https://raw.githubusercontent.com/zhichaoh/catppuccin-wallpapers/main/landscapes/mountains.png" -o "$wallpapers_dir/Doombox.png" || true
    curl -sL "https://raw.githubusercontent.com/zhichaoh/catppuccin-wallpapers/main/misc/cat_leaves.png" -o "$wallpapers_dir/Forest.png" || true
    curl -sL "https://raw.githubusercontent.com/zhichaoh/catppuccin-wallpapers/main/landscapes/mountains.png" -o "$wallpapers_dir/Horizon.png" || true

    # Validar que todos los archivos existan
    for r in "${available_themes[@]}"; do
        if [[ ! -s "$wallpapers_dir/$r.png" ]]; then
            cp "$wallpapers_dir/CatppuccinMocha.png" "$wallpapers_dir/$r.png" 2>/dev/null || true
        fi
    done
    cp "$wallpapers_dir/CatppuccinMocha.png" "$wallpapers_dir/current_wallpaper.png" 2>/dev/null || true

    fix_permissions
    log_success "Colección de Rices y Wallpapers descargada y lista."
}
...
# 1. Actualizar colores de bordes de BSPWM en caliente
...
# 2. Actualizar wallpaper del Rice activo
mkdir -p "$WALLPAPER_DIR"
WP_FILE="$WALLPAPER_DIR/${TARGET_THEME}.png"

# Si no existe, intentar descargarlo o buscar alternativa
if [[ ! -s "$WP_FILE" && -n "$WALLPAPER_URL" ]]; then
...
if [[ -f "$WP_FILE" && -s "$WP_FILE" ]]; then
    ...
    echo "feh --no-fehbg --bg-fill '$WP_FILE'" > "$HOME/.fehbg"
    chmod +x "$HOME/.fehbg"
else
    xsetroot -solid "${BG_COLOR:-#1e1e2e}"
fi

# 3. Guardar estado actual
echo "$TARGET_THEME" > "$CURRENT_RICE_FILE"

# 4. Regenerar configuraciones dinámicas de Polybar, Kitty, Dunst y Rofi
~/.local/bin/apply_theme_configs "$TARGET_THEME"

# 5. Reiniciar Polybar y Dunst para aplicar la nueva paleta de colores
...
#!/usr/bin/env bash
USER_HOME="${HOME}"
THEME_NAME="${1:-$(cat "$USER_HOME/.config/bspwm/current_rice" 2>/dev/null || echo 'CatppuccinMocha')}"
THEME_ENV="$USER_HOME/.config/bspwm/rices/$THEME_NAME/theme.env"

[[ -f "$THEME_ENV" ]] && source "$THEME_ENV"

mkdir -p "$USER_HOME/.config/polybar" "$USER_HOME/.config/kitty" "$USER_HOME/.config/dunst" "$USER_HOME/.config/rofi"

# 1. Polybar colors.ini (Colores dinámicos del Rice activo)
cat << EOPOLYBAR > "$USER_HOME/.config/polybar/colors.ini"
[colors]
base        = ${BG_COLOR:-#1e1e2e}
surface0    = ${SURFACE0:-#313244}
surface1    = ${SURFACE1:-#45475a}
surface2    = ${SURFACE2:-#585b70}
text        = ${FG_COLOR:-#cdd6f4}
subtext0    = ${SUBTEXT:-#a6adc8}
accent      = ${ACCENT_COLOR:-#89b4fa}
alt_accent  = ${ALT_ACCENT:-#cba6f7}
blue        = ${ACCENT_COLOR:-#89b4fa}
mauve       = ${ALT_ACCENT:-#cba6f7}
green       = ${GREEN:-#a6e3a1}
...
# 2. Kitty config
cat << EOKITTY > "$USER_HOME/.config/kitty/kitty.conf"
...
foreground              ${FG_COLOR:-#cdd6f4}
background              ${BG_COLOR:-#1e1e2e}
cursor                  ${ACCENT_COLOR:-#89b4fa}
cursor_text_color       ${BG_COLOR:-#1e1e2e}
selection_background    ${SURFACE1:-#45475a}
selection_foreground    ${FG_COLOR:-#cdd6f4}

active_border_color     ${ACCENT_COLOR:-#89b4fa}
inactive_border_color   ${SURFACE1:-#45475a}

# ANSI Colors
color0  ${SURFACE1:-#45475a}
color8  ${SURFACE2:-#585b70}
color1  ${RED:-#f38ba8}
color9  ${RED:-#f38ba8}
color2  ${GREEN:-#a6e3a1}
...
color4  ${ACCENT_COLOR:-#89b4fa}
color12 ${ACCENT_COLOR:-#89b4fa}
color5  ${ALT_ACCENT:-#cba6f7}
color13 ${ALT_ACCENT:-#cba6f7}
color6  ${TEAL:-#94e2d5}
color14 ${TEAL:-#94e2d5}
color7  ${FG_COLOR:-#cdd6f4}
color15 #ffffff
EOKITTY

# 3. Dunst config
cat << EODUNST > "$USER_HOME/.config/dunst/dunstrc"
[global]
    monitor = 0
    follow = mouse
    width = 320
    ...
    frame_width = 2
    frame_color = "${ACCENT_COLOR:-#89b4fa}"
    gap_size = 10
...
[urgency_critical]
    background = "${BG_COLOR:-#1e1e2e}"
    foreground = "${RED:-#f38ba8}"
    frame_color = "${RED:-#f38ba8}"
    timeout = 0
EODUNST
EOF
...
CURRENT_RICE=$(cat "$HOME/.config/bspwm/current_rice" 2>/dev/null || echo "CatppuccinMocha")
POLYBAR_CFG="$HOME/.config/bspwm/rices/$CURRENT_RICE/polybar.ini"

if type "xrandr" > /dev/null; then
  for m in $(xrandr --query | grep " connected" | cut -d" " -f1); do
    MONITOR=$m polybar --reload --config="$POLYBAR_CFG" main &
  done
else
  polybar --reload --config="$POLYBAR_CFG" main &
fi
EOF
    chmod +x "$poly_script"
...
EOF
    ...
    # 7.3 Polybar Modules Config
    local polybar_modules="$TARGET_HOME/.config/polybar/modules.ini"
    cat << 'EOF' > "$polybar_modules"
; ==============================================================================
; Polybar Modules - Multi-Rice Adaptable
; ==============================================================================
...
format-background = ${colors:alt_accent}
format-padding = 1
...
label-active-underline= ${colors:alt_accent}
...
time = %H:%M:%S
format-prefix = "󰥔 "
format-prefix-foreground = ${colors:alt_accent}
label = %date% %time%
...
EOF
    ...
    # Garantizar permisos antes y después de aplicar configs
    fix_permissions
    HOME="$TARGET_HOME" sudo -u "$TARGET_USER" "$TARGET_HOME/.local/bin/apply_theme_configs" "CatppuccinMocha" || true
    fix_permissions
